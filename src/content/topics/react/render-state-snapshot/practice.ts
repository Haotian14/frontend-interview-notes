...
export const practice: TopicPractice = {
  ...
  code: {
    input: '在同一个点击处理函数里连续调用三次 `setCount(count + 1)`，再换成三次 `setCount(previous => previous + 1)` 对比。',
    output: '第一种写法三次都读同一个快照，计数只从 0 变到 1；第二种按队列依次接收前一个结果，得到 3。',
  },
  reference: {
    caption: 'React 状态选择',
    rows: [
      { term: '渲染期计算', meaning: '可由现有 props 与 state 推导的值。' },
      { term: '独立 state', meaning: '无法从现有数据纯计算的交互状态。' },
      { term: '函数式更新', meaning: '下一状态依赖前一状态时使用。' },
      { term: 'key 变化', meaning: '建立新组件身份，重置该子树状态。' },
    ],
  },
  interview: {
