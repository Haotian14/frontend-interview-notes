...
// 正文索引现在是独立分片、异步加载；测试里一次取回后同步使用。
const sections = await loadSearchSections();
const search = (query: string) => searchTopics(query, sections);

describe('weighted topic search', () => {
  test('serves metadata hits before the body index arrives', () => {
    // 索引还没到时传空数组：标题和关键词仍然可搜，只是没有正文小节。
    const results = searchTopics('事件循环', []);
    expect(results[0]?.item.slug).toBe('event-loop');
    expect(results.every(result => result.section === undefined)).toBe(true);
  });

  test('ranks title matches above body and summary matches', () => {
    const results = search('事件循环');
    expect(results[0]?.item.slug).toBe('event-loop');
  });
  ...
  test('finds several rendering topics without loading article modules', () => {
    const slugs = search('渲染').map(result => result.item.slug);
  ...
  test('body hits carry a section anchor for deep linking', () => {
    const hit = search('游离').find(result => result.item.slug === 'closures-scope');
  ...
  test('metadata matches open the topic itself, without a section anchor', () => {
    ...
    // 不该把人直接丢到正文中段。
    const hit = search('缓存').find(result => result.item.slug === 'http-cache');
